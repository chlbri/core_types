...
import { TestActual, TestExpected, TestTable } from "./types";
...
const actuals1f1: TestActual<ForTest1, Length1> = [
  [5, 6],
  [1, 0],
  [15, 37],
  [105, 100],
...
const actuals1f2: TestActual<ForTest2, Length1> = [
  [-1],
  [1],
  [-15],
  [105],
...
const actuals2f1: TestActual<ForTest1, Length2> = [
  [5, 6],
  [15, 37],
  [24, 7],
];

const actuals2f2: TestActual<ForTest2, Length2> = [[-5], [-15], [24]];

const expecteds1: TestExpected<Length1> = [
  false,
  true,
  false,
  true,
...
describe("Generate the Table of Tests", () => {
  ...
  it("For 3 elements", () => {
    ...
    const expected: TestTable<ForTest1, Length2> = [
      [[5, 6], false],
      [[15, 37], false],
      [[24, 7], true],
    ];
...
describe("Map all Tests", () => {
  (() => {
    return describe(`Function  ==>   ${dumpFunction1}`, () => {
      const spy = jest.fn(mapperTest);
      const func = spy(dumpFunction1);
  ...
  (() => {
    return describe(`Function  ==>   ${dumpFunction2}`, () => {
      const spy = jest.fn(mapperTest);
