...
declare type PromisifyObject<T extends object> = T & {
    [P in keyof T & string as PromisifyMethod<T[P]> extends never ? never : `${P}Async`]: PromisifyMethod<T[P]>;
};
declare type WithoutPassword<T> = Omit<T, "password">;
declare type OnlyNamesOf<T, TProp> = {
    [K in keyof T]: Exclude<T[K], undefined> extends TProp ? K : never;
}[keyof T];
declare type OnlyPropertiesOf<T, TProp> = Pick<T, OnlyNamesOf<T, TProp>>;
declare type Email = `${string}@${string}.${string}`;
declare type MinLetters = string;
declare type MaxLetters = string;
declare type NExtract<T, U extends T> = Extract<T, U>;
declare type NExclude<T, U extends T> = Exclude<T, U>;
export { IndexOfArray, Awaited, TupleOf, Reverse, AddString, TuplifyUnion, JoinString, CoreDataBaseSchema, DomainUseCaseSchema, Digit, OnPropChangedMethods, PromisifyMethod, PromisifyObject, WithoutPassword, OnlyNamesOf, OnlyPropertiesOf, Email, MinLetters, MaxLetters, NExtract, NExclude, };
